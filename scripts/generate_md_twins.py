...
BASE_URL = "https://darrynpasqua.com"

SHORTCODE_BLOCK = re.compile(r"\{%-?\s*\w+\(.*?\)\s*-?%\}.*?\{%-?\s*end\s*-?%\}", re.DOTALL)
SHORTCODE_INLINE = re.compile(r"\{\{-?\s*\w+\(.*?\)\s*-?\}\}")
...
def clean_body(body: str) -> str:
    body = SHORTCODE_BLOCK.sub("", body)
    body = SHORTCODE_INLINE.sub("", body)
...
def extract_rendered_text(out_dir: Path, slug: str) -> str:
    """Fallback for pages built mostly from shortcodes: extract text from rendered HTML."""
